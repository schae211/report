...
* The runtimes for the `synthetic` datasets with one paraview (`l=12`) show that LM is the fastest model followed by MARS. In the middle there are TBOOST and RF which are equally fast. The slowest models are BGMARS, SVM and MLP (which is by a large margin the slowest model). However, when looking at these time we should not forgest that they do not provide information about scalability. For example the MARS model with the default parameters (`degree = 2, k = 5, thres=0.01`) is faster than RF on the synthetic dataset which has 10 samples, with approximately 4000 cells and 11 markers in each sample, but MARS is much slower on datasets with more markers as described in the next paragraph.
