...
```{r}
rf.result.files <- map2_chr(misty.views.smp, names(misty.views.smp), 
                               function(misty.view, smp) {
  run_misty(misty.view, results.folder = paste0("output/case_study_B/RF", smp))
})
rf.results <- collect_results(rf.result.files)
```
...
```{r message=FALSE, warning=FALSE}
nnls.result.files <- map2_chr(misty.views.smp, names(misty.views.smp), 
                               function(misty.view, smp) {
  run_misty(misty.view, results.folder = paste0("output/case_study_B/NNLS", smp),
            model.function = nnls_model)
})
nnls.results <- collect_results(nnls.result.files)
```
