...
Function to compute the cosine similarity for a given view.

```{r}
cosine_similarity <- function(keys, splits, sel.view, relative="RF") {
  # 1. get the tibbles corresponding to the selected view
  keys %>%
    mutate(id = row_number()) %>%
    filter(view == sel.view) %>%
    pull(id, name = sample) %>%
    # 2. map over each sample (tibble) and bind the results as rows
    map_dfr(function(tbl.id) {
      
      tbl <- splits[[tbl.id]]
      
      # 3. get the random forest importance vector as reference
      rf <- tbl %>% filter(algorithm == relative) %>% 
        select(-c(sample, view, algorithm)) %>%
        unlist()
      
      # 4. get the importance vectors for the other algorithms by mapping over rows
      cosine.sims <- slide_dfr(tbl, function(row) {
        vec <- row %>% select(-c(sample, view, algorithm)) %>%
          unlist()
        
        # 5. for each importance vector compute the cosine similarity
        cosine.sim <- (rf %*% vec * 1/(rf %*% rf)**0.5 * 1/(vec %*% vec)**0.5) %>%
          as.double()
        
        tibble(algorithm = row %>% pull(algorithm), cosine = cosine.sim, 
               sample = row %>% pull(sample))
      })
    }) %>%
    # 6. Group by algorithm and get the mean of the cosine similarity for each sample
    group_by(algorithm) %>%
    summarise(mean = mean(cosine), sd = sd(cosine))
}
```
...
Split the importances according to view and sample.
...
Now let's look at the cosine similarities for the intraview for each sample 
(and summarize using the mean and the standard deviation).

```{r}
cosine_similarity(keys, splits, "intra")
```

#### Paraview

```{r}
cosine_similarity(keys, splits, "para.6")
```
...
```{r}
cosine_similarity(keys, splits, "juxta.40")
```

#### Paraview

```{r}
cosine_similarity(keys, splits, "para.120")
```
