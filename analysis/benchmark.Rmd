 ...
 * Multi R2 (proportion of variance explained) and Gain in R2 are compared between the different algorithms of each dataset.

3. [Comparing Importances]

 * Cosine Similarity of the importance vectors are compared between the different algorithms for each view in each dataset. Additionally the jaccard index after binarization of the importantes is used for comparison.
...
handle such large datasets with 100 markers and up to 70,000 spatial units. Even if an approximation of 60% is used, only 4 of 36 samples could be processed. (thus the name `MARS60`)
...
Since the bagged MARS algorithm could only finish the computation for two samples I wll exclude the result. The results from SVM and MLP will be removed as well.
...
Again the numbers `MARS80` and `MARS60` indicate that only 80/60% of the training data where to train the view-specific models. However, the performance does not seem to be worse in this case if we use these approximations.
...
Looking at the cosine similarity we see that the importances returned by the MARS model and by the bagged MARS model are essentially the same which we would expect. The importances returned by the MARS models, the RF, and the TBOOST are all very similar in both the intra- and the paraview. In the contrast, the values returned by the LM are very different, especially in the paraview. They are closer to SVM and LBOOST.
...
Here we see a similar clustering as for the `synthetic` dataset, with TBOOST, BGMARS, MARS, and RF clustering together on one side, and LBOOST, LM, and SVM on the other side. Interestingly the overall cosine similarity between the different algorithms decreases going from the intra, to the juxta, to the paraview, as we can see by looking at the different scales.
...
For some reason there are some NAs in the models based on gradient boosting.
...
Again, we can observe the same trend as in the other datasets. TBOOST, MARS, and RF cluster together on one side, and LM and LBOOST on the other side.
...
Since not all sample could be processed by BGMARS, MLP, and SVM, we should exclude them from the analysis.

```{r}
filter_runs(all.results, "merfish_bc", "hvg") %>%
  get_importance() %>%
  filter(!(algorithm %in% c("BGMARS", "SVM", "MLP"))) %>%
  check_algs()
```

Interestingly, ther is a different trend in this datasets compared to the other datasets. RF is more dimilar to LBOOST than to MARS and TBOOST in the intraview. And in the juxta- and paraview the importances of the different models are very different.
...
We can clearly see that `max.depths = 1` reduces the performance significantly. Apart from that the model seems very robust to hyperparameter changes which is another reason why random forests are so popular.
...
of hyperparameters here, hinting that it might be ok to reduce `fast.k` from 20 to 5.
...
Since the runtime is the biggest issue for the MLP model, I tested all backpropagation implemented in `RSNNS` but this did not really help to increase the runtime very much. In fact the default algorithm "Std_Backpropagation" is one of the fastest.
...
map_dfr(funcs, function(func) {
   ...
   start <- Sys.time()
   considered.views <-
     all.views$`/synthetic/l12`[as.numeric(str_extract(
       names(all.views$`/synthetic/l12`), "[0-9]+")) <= 5]
   walk(considered.views, function(view) {
