...
where each column represents a vector. Indices are harcoded.
...
Function to plot heatmap of the cosine similarity of the jaccard index
after binarization of the importances.
...
cosine_heatmap <- function(misty.importances) {
  ...
  plots <- map(views, function(v) {
    ...
    tmp.df <- tidy.df %>%
      ...
      mutate(sample = str_extract(sample, "[^/]+$")) %>%
      group_by(algorithm) %>%
      summarise(across(4:(ncol(.)-1), mean)) %>%
      replace(is.na(.), 0) %>%
...
Now let's look at the cosine similarities, pearson correlation, and 
the jaccard indices after binarization.
...
increases the performance here.
